...
## 2026-09-02 - The advice a second task also gave, and the provider that was never loaded

**The largest free allowance in the deployment was never in a CLI run's pool.**
`ProviderPool.from_env` defaulted `allow_data_training=False`, and only the
server read `SWARMD_ALLOW_DATA_TRAINING`. So a run launched from the dashboard
had Mistral -- 1B tokens a month, no daily cap -- and the same run launched from
a terminal did not, while `providers budget` printed "Enabled for this
deployment" the whole time. Sessions parked for hours on an exhausted OpenRouter
day with the capacity sitting unloaded.

This is exactly the CLI/service drift the PRR carries as a watch item, caught in
the act. Both consent gates now resolve in one place, defaulting to `None`
meaning "whatever the operator decided" rather than `False` meaning "no".
Probe went 4 providers to 5; the next 300 calls were 203 Mistral, 61 Groq, 36
Google.

**Distillation was the one place in this system that trusted instead of
verifying** -- ADR-015 named it and could not close it, because domain-as-method
contamination has no structural signature. `probes` and `monitoring` look
exactly like `parse` and `validate` to any rule that does not already know the
subject. A detector was built, measured (0.41/0.52/0.35 against 0.33) and
refused rather than tuned on four samples.

The instrument was already in the building. A skill only reaches a human once
two DIFFERENTLY-SHAPED tasks produced it, and each produced its own wording.
Contamination is one task's vocabulary by construction, so it cannot survive an
intersection with the other. No detector, no threshold, no lexicon of subjects:
a word is kept because more than one task shape used it (ADR-016).

**The first cut of that produced garbage, and the garbage was the useful part.**
Keeping one wording's grammar and deleting its unattested words gave:
...
Contamination gone, meaning gone with it. An intersection is a SET; filtering a
sentence produces text neither task wrote. Served as a set and labelled as one,
the same record reads:

```
When a step calls for work of this kind: 3 differently-shaped tasks described
this approach with: output. Produce a JSON object whose values are of these
kinds: str.
```

**Then the identity was measured again and reversed again.** ADR-014 keyed an
approach on its NAME, which is `produce ` plus the artifact's field names --
and field names are chosen by a worker for one task's criterion. On a 38-record
library distilled entirely after the ADR-016 fixes: **38 approaches, every one
a singleton.** Nothing could ever accumulate.

Keyed on the artifact SHAPE instead -- field count plus sorted value kinds,
both generated by this system's own distiller -- the same library gives 28
approaches and 8 holding more than one record.

A key that loose would have been poison a day ago. Four of those eight groups
share a type signature and no method whatever, and corroborate to zero terms.
Now `promotable` refuses a record whose wordings agree on nothing, so the bad
merges never reach a reviewer and the good ones finally can. **The looser key
is affordable precisely because corroboration polices it.**

**Where that leaves G-4, stated exactly.** The loop distils, merges,
corroborates, and then declines to promote: the two records clearing the
evidence bar corroborate to the single word `output`, and approving those would
put noise in a retrieval slot. That is the mechanism working, not failing.

The remaining blocker is nameable and it is NOT model capability: members of a
task family produce DIFFERENT artifact shapes, because each task's criterion is
authored fresh (ADR-009) and the worker picks its own fields. Cross-task shape
matches do happen -- 2 of 8 groups -- just rarely. Volume is the lever, which
is what "Next up" already said, and it now costs time rather than quota.

Also today: a torn row in `usage.jsonl`, left by two sessions appending at once,
was re-warned on every budget read and buried the pacer's output under hundreds
of identical lines. Reported once per load now. And the preflight timeline says
which allowances it could not count -- it excludes rate-kind providers by
