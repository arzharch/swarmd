...
### G-4 · Update, 2026-09-02: the loop now declines to promote, and that is the mechanism working

Two more faults were traced and closed, and the remaining blocker is nameable.

**Contamination is closed by evidence, not by a detector (ADR-016).** A skill
only reaches a human once two DIFFERENTLY-SHAPED tasks produced it, and each
produced its own wording. Domain-as-method contamination is one task's
vocabulary by construction, so it cannot survive an intersection with the
other. What a worker is shown is the vocabulary every recorded wording used --
served as a labelled term set, because filtering a sentence produces text
neither task wrote.

**Identity was keyed on worker-chosen field names, and that was measured to be
fatal.** `approach: produce diagnosis, fix` against `produce problem,
fixed_query_filter, records_found`: same work, two records, neither
accumulating. On a 38-record library distilled entirely after the ADR-016
fixes, name identity produced **38 approaches, every one a singleton**. Keyed
on the artifact SHAPE -- field count plus sorted value kinds, both generated by
this system's own distiller -- the same library gives 28 approaches and 8
holding more than one record. A key that loose would have been poison before
corroboration existed; `promotable` now refuses a record whose wordings agree
on nothing, so the bad merges never reach a reviewer.

**The honest state of the claim.** The loop distils, merges, corroborates, and
then declines: the records clearing the evidence bar corroborate to the single
word `output`, and approving those would put noise in a retrieval slot for no
benefit. Nothing is approved, so no improvement is claimed.

**The blocker is not model capability.** Members of a task family produce
DIFFERENT artifact shapes, because each task's criterion is authored fresh
(ADR-009) and the worker chooses its own fields. Cross-task shape matches do
occur -- 4 of 31 shapes at 41 records -- just rarely, and each one is a
candidate the loop can actually be asked about. Volume is the lever, and with
Mistral loaded it costs time rather than quota.
